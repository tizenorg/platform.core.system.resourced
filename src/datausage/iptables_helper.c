...
/*
 [0:0] -A RESD_OUT -m cgroup --cgroup 5 -j RETURN
*/
...
int SetupChains()
{
    ...
    /* output traffic */
    ret = snprintf(cmd_buf, sizeof(cmd_buf), "%s -N %s", iptables_cmd, resourced_output_chain);
    ...
    ret = snprintf(cmd_buf, sizeof(cmd_buf), "%s -A %s -j %s", iptables_cmd, out_chain, resourced_output_chain);
    if (ret >= sizeof(cmd_buf))
    {
        _D("buffer too small\n");
        return -1;
    ...
    /* input traffic */
    ret = snprintf(cmd_buf, sizeof(cmd_buf), "%s -N %s", iptables_cmd, resourced_input_chain);
    if (ret >= sizeof(cmd_buf))
    {
        _D("buffer too small\n");
        return -1;
    ...
    ret = snprintf(cmd_buf, sizeof(cmd_buf), "%s -A %s -j %s", iptables_cmd, in_chain, resourced_input_chain);
    if (ret >= sizeof(cmd_buf))
    {
        _D("buffer too small\n");
        return -1;
...
int AddRuleForCgroup(int cgroup_num)
{
    ...
    /* output traffic */
    ret = snprintf(cmd_buf, sizeof(cmd_buf), "%s -A %s -m cgroup --cgroup %d -j RETURN", iptables_cmd, resourced_output_chain, cgroup_num);
    if (ret >= sizeof(cmd_buf))
    {
        _D("buffer too small\n");
        return -1;
    ...
    /* input traffic */
    ret = snprintf(cmd_buf, sizeof(cmd_buf), "%s -A %s -m cgroup --cgroup %d -j RETURN", iptables_cmd, resourced_input_chain, cgroup_num);
    if (ret >= sizeof(cmd_buf))
    {
        _D("buffer too small\n");
        return -1;
...
int GetCgroupCounters( struct counter_arg *carg)
{
    ...
    ret = snprintf(cmd_buf, sizeof(cmd_buf), "%s -t filter -c ", iptables_save_cmd);
    if (ret >= sizeof(cmd_buf))
    {
        _D("buffer too small\n");
        return -1;
...
int ZeroCounters()
{
    ...
    /* output traffic */
    ret = snprintf(cmd_buf, sizeof(cmd_buf), "%s -Z %s ", iptables_cmd, resourced_output_chain);
    if (ret >= sizeof(cmd_buf))
    {
        _D("buffer too small\n");
        return -1;
    ...
    /* input traffic */
    ret = snprintf(cmd_buf, sizeof(cmd_buf), "%s -Z %s ", iptables_cmd, resourced_input_chain);
    if (ret >= sizeof(cmd_buf))
    {
        _D("buffer too small\n");
        return -1;
