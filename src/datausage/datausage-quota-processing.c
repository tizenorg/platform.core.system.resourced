	...
	const resourced_roaming_type roaming)
{
	...
	_SD("Get counted traffic for appid:%s, per"
		...
		(int)out_context.rcv_used_quota, (int)out_context.sent_used_quota);
...
static void check_and_clear_warning_noti(void)
{
	int *warning_noti_id;

	warning_noti_id = get_noti_id();

	_D("data_warning: %d", *warning_noti_id);

	if (*warning_noti_id) {
		clear_datausage_noti(*warning_noti_id);
	}
...
static const int64_t quota_gap_value[RESOURCED_IFACE_ALL] = {
	4500000,	/* ~4.5MB UNKNOWN */
	3000000,	/* ~3MB RESOURCED_IFACE_DATACALL */
	6000000,	/* ~6MB RESOURCED_IFACE_WIFI */
	100000000,	/* ~100MB RESOURCED_IFACE_WIRED */
...
int _is_under_restriction(const int64_t send_delta,
	...
	const resourced_iface_type iftype)
{
	const int64_t quota_gap = _get_quota_gap(iftype);
...
{
	ret_value_msg_if(!app_quota, ,"Please provide valid pointer");
				     ...
				     gpointer value, gpointer UNUSED user_data)
{
	...
	if (_is_under_restriction(send_delta, rcv_delta, key_quota->iftype) &&
		...
		if (process_restriction_local(key_quota->app_id,
...
static void check_and_apply_quota(void)
{
	g_tree_foreach(quotas, check_and_apply_node, NULL);
}

struct update_all_arg
{
	resourced_iface_type iftype;
	const struct application_stat *app_stat;
};

static gboolean update_pseudo_app_entry(gpointer key,
	gpointer value, gpointer user_data)
{
	const struct update_all_arg *arg = (const struct
	...
	if ((!strcmp(qkey->app_id, RESOURCED_ALL_APP) &&
		...
		total_quota->sent_used_quota += arg->app_stat->snd_count;
		total_quota->rcv_used_quota += arg->app_stat->rcv_count;
	}
...
static void update_traffic_quota(const struct quota_key *quota_key,
				 const uint32_t snd_count,
				 const uint32_t rcv_count)
{
	...
	gpointer UNUSED userdata)
{
	...
	const struct application_stat *app_stat =
		(const struct application_stat *)value;
	...
	update_traffic_quota(&qkey, app_stat->snd_count,
			     app_stat->rcv_count);
	return FALSE;
}
