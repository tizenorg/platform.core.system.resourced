...
resourced_ret_c get_storage_root_paths(int type, GSList **paths)
{
	...
	if (type == INTERNAL) {
		...
		while (!readdir_r(dp, &dentry, &result) && result != NULL) {
			if(dentry.d_name[0] == '.')
				continue;
			root_path = (char*)malloc(BUF_MAX);
			if(snprintf(root_path, BUF_MAX, "/home/%s/content", dentry.d_name) < 0) {
				_D("Fail to make root path of %s. This path will not be included", root_path);
				continue;
			}
			if (!opendir(root_path)) {
